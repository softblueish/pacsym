...
func main() {
	...
	if cap(os.Args) != 1 {
		...
		// When the user types sync, it is to find all packages in the /usr/pkg/ directory and
		// symlinks them according to its hiearchy
		case "sync":
			...
			// This for loop will make sure the directory of every package in /usr/pkg is 
			// catalogued in the packagePath array
			for _, f := range files {
				packagePath = append(packagePath, "/usr/pkg/"+f.Name())
			}

			// This nested for loop will look in every package with the help of the packagePath
			// array and add the version of the package to the packagePath array
			//
			...
			for i := 0; i < len(packagePath); i++ {
				...
				for _, f := range files {
					...
					// When we reach this point in the for loop, we will index every file in the directory
					// who are elements in the packagePath array and place them in the filepath array
					filepath.WalkDir(packagePath[i], walk)
				}
			}

			// The for loop described under will tear appart every element in filePath, remove
			// /usr/pkg/<PACKAGENAME>/<PACKAGEVERSION>/ and make it an element of trueFilePath
			// An example would be the element /usr/pkg/foobar/1.1/usr/bin/dependency.sh would
			// turn into just /usr/bin/dependency.sh
			
			for i := 0; i < len(filePath); i++ {
				trueFilePath = append(trueFilePath, "/"+strings.Join(strings.Split(filePath[i], "/")[5:], "/"))
			}

			// Now it's time for the main attraction, linking every element in filePath to trueFilePath, this will
			// recursively repeat for every element present in the array filePath
			//
		...
		// If the "build" option was chosen, the package manager will attempt to place a tarball in /usr/pkgsrc
		// extract it, configure it and finally, build it.
		case "build":

			// When building a package, it might require certian arguments to work correctly, that means everything
			// written after pacsym build <tarball> will be counted as a build argument, unless it's a pacsym argument
			buildarguments = os.Args[3:]

			// Here's the first usecase of the pacsym arguments, "--local" or "-l", this will check if it exists in
			// the buildarguments array. If it is present, it will just index the tarball, if its not, it will
			// download the url present with wget, then index it.
			if inList(buildarguments, "--local") || inList(buildarguments, "-l") {

				// This will check os.Args[2], which the program assumes to be the location of the tarball,
				// and index it to tarPath.
				tarPath = os.Args[2]

				// This will take tarPath and take the last word before it finds a /, which will leave just
				// the name of the tarball and index it to tarName.
				tarName = (strings.Split(tarPath, "/")[len(strings.Split(tarPath, "/"))-1])
			} else {

				// If the --local option wasnt specified, os.Args will assume that the thing after "build" 
				// written is the url to the tarball, then index that url to the url variable.
				url = os.Args[2]
				...
				// This will take the url, find the last segment of it which would be the name of the
				// tarball and index it.
			...
			// The directory it was extracted to will be indexed into buildddir, pacsym assumes the length of the
			// tarball extensions is 7 characters (example: .tar.xz and .tar.gz)
			extensionLength := 7
			...
			// Now pacsym will check if the package requires for it to be built in a separate build directory,
			// which would require --separate-builddir to be passed, or -s
		...
		case "install":
			...
			// When executing install, it requires a <PACKAGENAME> and <PACKAGEVERSION>. It will take these and index them.
			// It will also assume everything after written are install arguments.
			...
			// Now the packages will be installed, Where one of the default paramaters added by pacsym is
			// DESTDIR=/usr/pkg/packageName/packageVersion, which is the core of pacsyms functions.
			//
		...
		// If the clean option is passed, it will remove everything in /usr/pkgsrc
		// which is important as pacsym can't build a new package if the /usr/pkgsrc directory isn't empty.
		case "clean":
